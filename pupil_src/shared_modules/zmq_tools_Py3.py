...
Modifying to work with Python 3.5
print -> print()
socket.set and .setsockopt -> .set_string, .setsockopt_string
(Or, alternative: setsockopt(zmq.SUBSCRIBE, b"pupil"))
tcp://127.0.0.1:%s'%ipc_sub_port -> tcp://127.0.0.1:%s'%int(ipc_sub_port)

Only working with 'pupil' subscriptions?
'''
...
class Msg_Dispatcher(Msg_Streamer):
    ...
    def notify(self,notification):
        ...
        if notification.get('remote_notify'):
            self.send_string("remote_notify.%s"%notification['subject'],notification)
        elif notification.get('delay',0):
            self.send_string("delayed_notify.%s"%notification['subject'],notification)
        else:
            self.send_string("notify.%s"%notification['subject'],notification)
...
if __name__ == '__main__':
    ...
    log_monitor = Msg_Receiver(ctx,'tcp://127.0.0.1:%s'%int(ipc_sub_port),topics=('logging.',))
    notification_monitor = Msg_Receiver(ctx,'tcp://127.0.0.1:%s'%int(ipc_sub_port),topics=('notify.',))
    monitor = Msg_Receiver(ctx,'tcp://127.0.0.1:%s'%int(ipc_sub_port),topics=('pingback_test.3',))
    gaze_monitor = Msg_Receiver(ctx,'tcp://localhost:%s'%int(ipc_sub_port),topics=('gaze.',))
    pupil_monitor = Msg_Receiver(ctx,'tcp://localhost:%s'%int(ipc_sub_port),topics=('pupil.',))

    
    #you can also publish to the IPC Backbone directly.
    publisher = Msg_Streamer(ctx,'tcp://127.0.0.1:%s'%int(ipc_pub_port))
    sleep(1)
    ...
    def roundtrip_latency_pubsub():
        ...
        for x in range(100):
            sleep(0.003)
            t = time()
            publisher.send_string('pingback_test.3',{'subject':'pingback_test.3','index':x})
            monitor.recv()
            ts.append(time()-t)
        print( min(ts), sum(ts)/len(ts) , max(ts))
